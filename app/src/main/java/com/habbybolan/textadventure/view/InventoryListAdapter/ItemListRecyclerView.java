...
public class ItemListRecyclerView implements InventoryListRecyclerView{
    ...
    public ItemListRecyclerView(Context context, RecyclerView recyclerView, CharacterViewModel characterVM, InventoryClickListener inventoryClickListener) {
        // set the layout manager to position the items
        recyclerView.setLayoutManager(new LinearLayoutManager(context));

        adapter = new InventoryListAdapter(characterVM.getItems(), inventoryClickListener);
